#兼容主程序V1.0、V2.0、V3.0，本程序是计算赋分后的每位同学的排名（分为班级内部和全部）（独立的程序，请务必运行主程序再运行插件）
import pandas as pd
...
def state1(df_orignal):
    ...
    return df_orignal
df=pd.read_excel('成绩（结合首考）.xlsx')
if state==0:
    list=get_class_list(df)
    df1=state0(df,list)
    flag=True
elif state==1:
    df1=state1(df)
...
if flag==True:
    if state==1:
        df1.to_excel("赋分后学生排名.xlsx", index=False)
    elif state==0:
        df1.to_excel("赋分后学生排名(以班级独立排名).xlsx", index=False)
